...
async function installBinary({ repo, pattern, name, installDir, force, explicitUrl, log = () => {} }) {
    ...
    try {
        ...
        expandArchive(archive, extractDir);

        const found = findFileRecursive(extractDir, [name, 'sockseek.exe', 'sockseek', 'sldl.exe', 'yt-dlp.exe', 'yt-dlp', 'yt-dlp_linux', 'yt-dlp_macos']);
        ...
        if (fs.statSync(found).isFile() && fs.readdirSync(path.dirname(found)).length === 1) {
            // Un seul fichier dans son dossier (cas yt-dlp) : copie directe.
