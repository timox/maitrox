...
async function expandArchive(archivePath, destDir) {
    ...
    let result;
    try { result = await runCapture('tar', ['-xf', archivePath, '-C', destDir]); }
    catch (e) { throw new Error(`'tar' introuvable ou en echec : ${e.message}`); }
    if (result.status !== 0) {
        throw new Error(`echec de 'tar' sur ${archivePath} (code ${result.status}) : ${(result.stderr || '').trim()}`);
    }
}
...
async function getInstalledVersion(exePath) {
    try {
        const res = await runCapture(exePath, ['--version'], { timeoutMs: 8000 });
...
async function installBinary({ repo, pattern, name, installDir, force, explicitUrl, archive = true, log = () => {} }) {
    ...
    const alreadyPresent = fs.existsSync(exePath);

    if (alreadyPresent && !force && explicitUrl) {
        // Une URL explicite est fournie pour contourner le quota GitHub :
        // pas de version comparable a portee (pas d'appel a l'API), donc
        // pas de re-telechargement systematique -- -Force force quand meme.
        log('Deja present (URL explicite fournie -- -Force pour reinstaller).');
        return exePath;
    }

    let asset;
    if (explicitUrl) {
        asset = { version: 'fournie', name: path.basename(explicitUrl), url: explicitUrl };
    }
    else {
        try {
            asset = await getLatestAsset(repo, pattern);
        }
        catch (e) {
            // Deja installe : une panne de verification (quota GitHub, pas
            // de reseau) ne doit pas rendre indisponible un binaire qui
            // fonctionnait deja -- on garde la version en place plutot que
            // de faire echouer toute l'installation pour ca.
            if (alreadyPresent && !force) {
                log(`Verification de mise a jour impossible (${e.message.split('\n')[0]}) : version installee conservee.`);
                return exePath;
            }
            throw e;
        }
        if (alreadyPresent && !force) {
            ...
            const latest = String(asset.version).replace(/^v/, '');
            if (installed && installed.replace(/^v/, '') === latest) {
                log(`Deja a jour (version ${installed}).`);
                return exePath;
            }
            log(installed
                ? `Mise a jour disponible : ${installed} -> ${asset.version}.`
                : `Version installee inconnue, mise a jour vers ${asset.version}.`);
        }
    }
...
async function testSoulseekConnection(sockExe) {
    // Verifie la connexion automatiquement a la fin de l'installation,
    // plutot que de demander a l'utilisateur de lancer lui-meme cette
    // commande de test : la meme recherche que celle historiquement
    // suggeree (elle cree le compte Soulseek au passage si besoin).
    let res;
    try {
        res = await runCapture(sockExe, ['Sciahri - Let Them Go', '--song', '--print', 'results'], { timeoutMs: 60000 });
    }
    catch (e) {
        return { ok: false, message: `Impossible de lancer sockseek pour tester la connexion : ${e.message}` };
    }

    const output = `${res.stdout}\n${res.stderr}`;
    if (/INVALIDPASS/.test(output)) {
        return {
            ok: false,
            message: 'Connexion Soulseek refusee (INVALIDPASS) : ce pseudo est probablement deja pris par ' +
                "quelqu'un d'autre -- Soulseek ne cree un compte que si le pseudo est libre. Choisir un " +
                'pseudo different dans la configuration.',
        };
    }
    if (/login failed/i.test(output)) {
        return { ok: false, message: 'Connexion Soulseek echouee (identifiants refuses). Verifier sockseek.conf.' };
    }
    if (res.status !== 0) {
        return { ok: false, message: `sockseek a repondu avec le code ${res.status}.` };
    }
    return { ok: true, message: 'Connexion Soulseek etablie (compte cree si besoin).' };
}
