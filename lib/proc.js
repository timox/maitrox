...
function runCapture(cmd, args, opts = {}) {
    ...
    //
    // onStderrLine (optionnel) : appele pour chaque ligne complete recue
    // sur stderr, au fil de l'eau -- pour filtrer un flux verbeux (voir
    // bin/extract.js) et n'en remonter que les lignes utiles, sans pour
    // autant perdre la capture complete (toujours renvoyee dans stderr,
    // pour le diagnostic d'erreur).
    const { timeoutMs, onStderrLine, ...spawnOpts } = opts;
    return new Promise((resolve, reject) => {
        ...
        proc.on('close', (code) => {
            ...
            settled = true;
            clearTimeout(timer);
            if (onStderrLine && lineBuf) onStderrLine(lineBuf);
